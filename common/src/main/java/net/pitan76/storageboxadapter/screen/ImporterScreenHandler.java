...
import net.pitan76.storageboxadapter.block.ImporterBlockEntity;
import org.jetbrains.annotations.Nullable;

public class ImporterScreenHandler extends ExtendedScreenHandler {
    public Inventory inventory;
    public Inventory filter;
    public PlayerInventory playerInventory;
    public ImporterBlockEntity tile = null;

    public ImporterScreenHandler(int syncId, PlayerInventory playerInventory, PacketByteBuf buf) {
        ...
        if (NbtUtil.has(data, "x") && NbtUtil.has(data, "y") && NbtUtil.has(data, "z")) {
            ...
            tile = (ImporterBlockEntity) WorldUtil.getBlockEntity(player.getWorld(), PosUtil.flooredBlockPos(x, y, z));
        }
    }
